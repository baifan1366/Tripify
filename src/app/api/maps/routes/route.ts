  ...
  try {
    ...
    for (let batch = 0; batch < activities.data.length - 1; batch += 4) {
      ...
      const results = await Promise.all(activities.data.slice(batch, Math.min(batch + 4, activities.data.length - 1)).map(async (from, offset) => {
        ...
        if ([from.latitude, from.longitude, to.latitude, to.longitude].some((v) => v === null)) return null;
        try { return await calculateRoute(from, to, input.data.mode); } catch { return null; }
      }));
    ...
    return reply({ segments, partial, version: trip.data.version, calculatedAt: new Date().toISOString() });
