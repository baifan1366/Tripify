...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!process.env.GOOGLE_MAPS_SERVER_API_KEY) {
      console.error(
        "[tripify:maps] routes: GOOGLE_MAPS_SERVER_API_KEY missing",
      );
    ...
    const activities = await client
      ...
      .eq("trip_id", input.data.trip)
      .eq("day_number", input.data.day)
      .order("start_time")
      .order("id")
      .limit(61);
    if (activities.error) return reply({ error: "UNAVAILABLE" }, 503);
    if (activities.data.length > 60)
      return reply({ error: "TOO_MANY_ACTIVITIES" }, 422);
    const withCoords = activities.data.filter(
    ...
    for (let batch = 0; batch < activities.data.length - 1; batch += 4) {
      ...
      const results = await Promise.all(
        activities.data
          .slice(batch, Math.min(batch + 4, activities.data.length - 1))
          .map(async (from, offset) => {
            const to = activities.data[batch + offset + 1];
            if (
              [from.latitude, from.longitude, to.latitude, to.longitude].some(
                (v) => v === null,
              )
            ) {
            ...
            try {
              return await calculateRoute(from, to, input.data.mode);
            } catch (e) {
              partial = true;
    ...
    const latest = await client
      ...
      .eq("id", input.data.trip)
      .maybeSingle();
    if (latest.data?.version !== trip.data.version)
